...
async function main() {
  try {
    ...
    execute('az extension add --name containerapp --upgrade')
      ...
      .then(() => (deploy())).catch(function() { throw new Error("failure to execute cli commands") })
...
let execute = function (command) {
  return new Promise((resolve, reject) => {
    exec(command, (error, stdout, stderr) => {
      if (error) {
        ...
        console.log(`Ran cmd ${command} successfully at ${dateTime}
    `)
        resolve(stdout);
      }
    });
  ...
  //   var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
  //   var dateTime = date + ' ' + time;
  //   console.log(cmd + ` at ${dateTime}`) // test log
  //   console.log()
  ...
  //     var time = today.getHours() + ":" + today.getMinutes() + ":" + today.getSeconds();
  //     var dateTime = date + ' ' + time;
  //     console.log(`Ran at ${dateTime}
  //   `)
  //     if (error !== null) {
  //       console.log('exec error: ' + error + `at ${dateTime}`);
