...
function setInputs() {
  ...
  setInput("monitoring", "https://www.google.com, https://www.bing.com")
}
// END TEST CODE
...
const STEP_PCT = parseFloat(core.getInput('stepPct')) // step pct of traffic going to new revision
const STEP_TIME = parseFloat(core.getInput('stepTime')) // step time between traffic shifts
const FINAL_PCT = parseFloat(core.getInput('finalPct')) // final pct of traffic going to new revision
    ...
    for (const test of MONITORS) {
      ...
      if (response.ok) {
        ...
        try {
          ...
          console.log(`Could not parse response JSON from test ${test}`)
        }
      } else {
