...
async function main() {
  try {
    ...
    const addExt = execute('az extension add --name containerapp --upgrade')

    // create a new revision
    const createRev = addExt.then(() => execute(`az containerapp update -n ${APP} -g ${RG} --revision-suffix ${REV_SUFFIX} --image ${IMAGE}`), () => fail("Failure to add containerapp extension"))
